...
export default function UploadFiles() {
  ...
  const traverseFileTree = async (item: any, path: string = ''): Promise<FileWithPreview[]> => {
    return new Promise((resolve, reject) => {
      if (item.isFile) {
        item.file((file: File) => {
          resolve([{ ...file, relativePath: path + file.name, preview: URL.createObjectURL(file) } as FileWithPreview]);
        });
  ...
  const onDrop = async (acceptedFiles: File[]) => {
    const filePromises = acceptedFiles.map((file) => {
      if ((file as any).webkitGetAsEntry) {
        ...
        return Promise.resolve([{ ...file, relativePath: file.name, preview: URL.createObjectURL(file) } as FileWithPreview]);
      }
    });
