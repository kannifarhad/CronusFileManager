...
import { 
  EditImage, FolderType, Items, ContextMenuTypeEnum, ViewTypeEnum, 
  OrderByType, ImagesThumbTypeEnum, HistoryType, HistoryStepTypeEnum, 
...
import { 
  getFilesList, copyFilesToFolder, cutFilesToFolder, deleteItems, emptydir, 
  createNewFile, createNewFolder, renameFiles, duplicateItem, saveFile, 
  uploadFile, unzip, archive 
...
export const useFileManagerOperations = ({ dispatch , selectItemCallback }: {dispatch: any, selectItemCallback: ((filePath: string) => void) | undefined}): Operations => {
  ...
  const handleApiError = useCallback((error: any, errorMessage: string) => {
    dispatch({
  ...
  const operations:Operations = useMemo(() => ({
    ...
    handleContextClick: ({ item, event, menuType }: { item: Items | null, event: React.MouseEvent, menuType: ContextMenuTypeEnum }) => {
      ...
      dispatch({
        ...
        payload: { item, mouseX: event.clientX - 2, mouseY: event.clientY - 4, menuType }
      });
    },
    ...
    handleGetInfo: (selectedFile: Items) => {
      ...
      dispatch({
        ...
        payload: {
          ...
          description:<> 
              <ul className="list">
                <li><b>Name</b> : {selectedFile.name}</li>
                <li><b>Path</b> : {selectedFile.path}</li>
                {selectedFile.type === ItemType.FILE && <>
                  <li><b>Size</b> : {formatBytes(selectedFile.size)}</li> 
                  <li><b>Extension</b> : {selectedFile.extension}</li>
                </>}
                <li><b>Created</b> : {convertDate(selectedFile.created)}</li>
                <li><b>Modified</b> : {convertDate(selectedFile.modified)}</li>
                <li>
                  <b>Permissions</b> : 
                  Others - {selectedFile.premissions.others}, 
                  Group - {selectedFile.premissions.group}, 
                  Owner - {selectedFile.premissions.owner}
                </li>
              </ul>
              {isImage && <img style={{maxWidth:'400px', maxHeight:'400px'}} alt={selectedFile.name} src={`${mainconfig.serverPath}${selectedFile.path}`} />}
            </>,
          handleClose,
        },
      });
    ...
    handlePreview: (selectedFile: FileType) => {
      ...
      dispatch({
        ...
        payload: {
          title:`File: ${selectedFile.name}`,
          description: <img style={{maxWidth:'400px', maxHeight:'400px'}} alt={selectedFile.name} src={`${mainconfig.serverPath}${selectedFile.path}`} />,
          handleClose,
        },
      });
    },
